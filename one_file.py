...
class Collector:
    ...
    def collect(self):
        ...
        if level > 0.5:
            if self.instance_settings[0]['num_orders'] > 10:
                random_num = random.random()
                if random_num < 0.3:
                    self.instance_settings[0]['num_types'] += 1
                    self.instance_settings[0]['num_types'] = min(5, self.instance_settings[0]['num_types'])
                elif random_num < 0.6:
                    self.instance_settings[0]['num_stages'] += 1
                    self.instance_settings[0]['num_stages'] = min(5, self.instance_settings[0]['num_stages'])
                else:
                    self.instance_settings[0]['num_machines_per_stage'] += 1
                    self.instance_settings[0]['num_machines_per_stage'] = min(5, self.instance_settings[0]['num_machines_per_stage'])
                self.instance_settings[0]['num_orders'] = 5
            else:
                self.instance_settings[0]['num_orders'] += 1

            self.last_total_rewards.clear()
            torch.save(self.old_policy_net.state_dict(), 'old_policy_net_0723.pth')
