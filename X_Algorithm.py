    ...
    def step(self, transition_name):
        ...
        if self.orders and self.orders[-1]['arrival_time'] < next_pn_state.cur_time:
            ...
            if (transition_name.startswith('end') and
                ...
                reward += (self.cur_pn_state.cur_time <= self.due_time_of_order_[
        ...
        self.num_done_orders += reward
        # print("达成率:", self.num_done_orders / self.num_received_orders)
        return self.cur_pn_state, reward, done, info
...
class SchedulingAlgorithm:
    def __init__(self):
        ...
        state_dict = torch.load('model_checkpoint_0728_ins1_metric-1051.pth')
        model = PNCN(
